...
class SARSALambdaTable:
    def __init__(self, actions, learning_rate=0.01, decay_rate=0.9, epsilon_greedy=0.9, decay_trace=0.9):
        # This function initializes the SARSA-Lambda parameters
        ...
        self.Lambda = decay_trace
        self.eligibility = self.table.copy()
    ...
    def learn(self, s_current, a_current, r, s_next, a_next):
        # This function implements the learning updates in terms of SARSA-Lambda
        ...
        prediction = self.table.ix[s_current, a_current]
        # Target calculation using the state / action pair we ended up in
        target = r + self.y * self.table.ix[s_next, a_next]

        # Basic Error Calculation
        err = target - prediction

        # Eligibility Track counter for each state / action pair
        self.eligibility.ix[s_current, a_current] += 1 # increase trace amount for visited state-action pair

        # SARSA-Lambda Update to the current state / action pair
        self.table += self.lr * err * self.eligibility

        # Decay the eligibility trace after the update
        self.eligibility *= self.y*self.Lambda
    ...
    def check_state(self, state):
        ...
        if state not in self.table.index:
            ...
            pending = pd.Series([0] * len(self.action_list), index=self.table.columns, name=state)
            self.table = self.table.append(pending)
            # Update the eligibility trace as well
            self.eligibility = self.eligibility.append(pending)
...
class ColossusControl(base_agent.BaseAgent):
    def __init__(self):
        ...
        self.SARSALambdalearn = SARSALambdaTable(actions=list(range(len(action_set))))
    ...
    def step(self, obs):
        ...
        # Choose an action from the SARSA-Lambda algorithm
        agent_action = self.SARSALambdalearn.choose_action(str(current_state))
        ...
        if self.prev_action is not None:
            ...
            if death_score > self.prev_death_score:
                reward += DEATH_REWARD    
            self.SARSALambdalearn.learn(str(self.prev_state), self.prev_action, reward, str(current_state), agent_action)
