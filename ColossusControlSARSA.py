...
class SARSATable:
    def __init__(self, actions, learning_rate=0.01, decay_rate=0.9, epsilon_greedy=0.9):
        # This function initializes the SARSA parameters
    ...
    def learn(self, s_current, a_current, r, s_next, a_next):
        # This function implements the learning updates in terms of SARSA
        ...
        # SARSA Update to the current state / action pair
        self.table.ix[s_current, a_current] += self.lr * (target - prediction)
...
class ColossusControl(base_agent.BaseAgent):
    def __init__(self):
        ...
        self.SARSAlearn = SARSATable(actions=list(range(len(action_set))))
    ...
    def step(self, obs):
        ...
        # Choose an action from the SARSA algorithm
        agent_action = self.SARSAlearn.choose_action(str(current_state))
        smart_action = action_set[agent_action]
        
        # Give immediate reward to the agent for previous action & the newly chosen one 
        if self.prev_action is not None:
            ...
            if death_score > self.prev_death_score:
                reward += DEATH_REWARD    
            self.SARSAlearn.learn(str(self.prev_state), self.prev_action, reward, str(current_state), agent_action)
